	...
	testCases := []struct {
	...
	}{
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				...
				// service方法预期的输入与输出，输入应该无所谓？主要是你想要啥输出
				// login中的实现
				userService.EXPECT().Login(gomock.Any(), domain.User{
					Email:    "123@qq.com",
					Password: "123456aA!",
				}).Return(domain.User{
					Email:    "123@qq.com",
			...
			reqBuilder: func(t *testing.T) *http.Request {
				// 传给request的body
				body := bytes.NewBuffer([]byte(`{"email":"123@qq.com", "password": "123456aA!"}`))
			...
			wantCode:     http.StatusOK,
			wantResponse: Result{Code: 0, Msg: "登录成功"},
			verifyHeader: true,
		},
		{
			name: "bind data error",
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				// mock的userService
				userService := svcmocks.NewMockUserService(ctrl)
				codeService := svcmocks.NewMockCodeService(ctrl)
		...
		},
		{
			...
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				...
				userService := svcmocks.NewMockUserService(ctrl)
				codeService := svcmocks.NewMockCodeService(ctrl)
				userService.EXPECT().Login(gomock.Any(), domain.User{Email: "123@qq.com", Password: "123456aA!"}).Return(domain.User{}, service.ErrEmailOrPassWrong)
				return userService, codeService
			},
			reqBuilder: func(t *testing.T) *http.Request {
				// 传给request的body
			...
			wantCode:     http.StatusOK,
			wantResponse: Result{Code: 4, Msg: "邮箱或者密码错误"},
			verifyHeader: false,
		},
		{
			name: "系统错误",
			mock: func(ctrl *gomock.Controller) (service.UserService, service.CodeService) {
				...
				userService := svcmocks.NewMockUserService(ctrl)
				codeService := svcmocks.NewMockCodeService(ctrl)
				userService.EXPECT().Login(gomock.Any(), domain.User{Email: "123@qq.com", Password: "123456aA!"}).Return(domain.User{}, errors.New("系统错误"))
				return userService, codeService
			},
			reqBuilder: func(t *testing.T) *http.Request {
				// 传给request的body
